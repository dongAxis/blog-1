...
/*
NOTE:
$ lldb out.gn/x64.debug/v8_shell
> br set --name v8::internal::interpreter::InterpreterGenerator::DoLdar
> pr launch -- --trace_ignition_codegen
*/
...
- Turbofan as CompilationJob

```
[ entries to Compiler::Compile ]
- Runtime_CompileLazy, Runtime_SetCode
- JSFunction::GetLength, EnsureHasInitialMap
- Debug::PerformSideEffectCheck

[ entries to OptimizedCode ]
...
- Q. what's Runtime (follow execution path to the Runtime function)

- Q. Interpreter entry trampoline
...
- follow path turbofan without going through bytecode (Pipeline::NewCompilationJob ?)
- phi node resolution
- compile non top-level function path (parsing and compiling when it's called ?)
- de-optimization path
- type of compilation job and how they are triggered
