...
- [ ] Client EGL
  ...
  - Reference:
- [ ] How client passes hw-accel buffer to server
    ...
    - wlc_output_set_backend_surface =>
      ...
      - wlc_context_bind_to_wl_display =>
        - bind_to_wl_display (egl.c) => eglBindWaylandDisplayWL =>
          - dri2_bind_wayland_display_wl (with dri2_dpy->vtbl->authenticate (i.e. ??)) =>
  ...
  - redraw =>
    ...
    - eglSwapBuffers => ?? ... =>
      - dri2_wl_swap_buffers (as dri2_dpy->vtbl->swap_buffers) => dri2_wl_swap_buffers_with_damage =>
        - create_wl_buffer =>
          - dri2_surf->current->wl_buffer = wl_drm_create_prime_buffer(.. fd)
        - wl_surface_attach
        - wl_surface_damage
