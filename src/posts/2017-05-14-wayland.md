...
- [ ] Client EGL
  ...
  - acts on server's wl_drm global
  - Reference:
- [ ] How client passes buffer to server
  - [x] Shm (passing shared memory fd via sendmsg)
  - [ ] EGL texture/image ??
      ...
      - wlc_context(&output->context, &output->bsurface) =>
        - wlc_egl =>
          - create_context =>
            ...
            - eglCreateContext
            - create_surface_gbm => eglCreateWindowSurface => ??
            - eglMakeCurrent
            - eglSwapInterval(.. 1)
          - api->swap = swap ...
      - wlc_context_bind_to_wl_display =>
        - bind_to_wl_display (egl.c) => eglBindWaylandDisplayWL =>
          - dri2_bind_wayland_display_wl (from mesa) =>
            - dri2_dpy->wl_server_drm = wayland_drm_init =>
  ...
  - repaint =>
    ...
    - render_view for each output->visible =>
      ...
      - wlc_render_view_paint => view_paint (gles2.c) =>
        - surface_paint_internal => texture_paint(.. wlc_surface, wlc_geometry) =>
          - glBindTexture, ...??
          - glVertexAttribPointer(... vertices) (vertices are define from wlc_geometry (2d position/size))
...
[ wlc view callback ]
- ? =>
  - wlc_view_commit_state =>
    - WLC_INTERFACE_EMIT_EXCEPT(view.created ..) =>
        ...
        - dri2_initialize =>
          - dri2_initialize_wayland =>
            - dri2_initialize_wayland_drm =>
              ...
              - roundtrip (this will force binding wl_drm at this point)
