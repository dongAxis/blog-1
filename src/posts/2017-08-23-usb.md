...
- how exactly do they transport audio samples (midi bytes) ?
...
(ALSA layer to USB layer (following alsa pcm vtable))
- (open) snd_usb_playback_open => snd_usb_pcm_open =>
  - ubstream->runtime = snd_usb_hardware
  - setup_hw_info => ..
- (hw_param) snd_usb_hw_params =>
  - snd_pcm_lib_alloc_vmalloc_buffer =>
    - substream->runtime = __vmalloc
  ...
  - start_endpoints (start here for playback) =>
    - snd_usb_endpoint_start =>
      ...
      - usb_submit_urb =>
        ...
        - usb_hcd_submit_urb => ??
        - (TODO: what's the completion callback ? where did we set up ?)
- (trigger) snd_usb_substream_playback_trigger =>
  - (case SNDRV_PCM_TRIGGER_START)
    - prepare_data_urb = prepare_playback_urb ..
- (mmap) snd_pcm_lib_mmap_vmalloc => ??

- TODO
  - snd_pcm_period_elapsed (when usb layer finished transporting audio ???)
  - (polling ?)
  - (does ioctl do any essential thing ?)


- cf. alsa lib (userspace)
  - important-looking vtable: snd_pcm_hw_ops, snd_pcm_hw_fast_ops (in src/pcm/pcm_hw.c)
```
