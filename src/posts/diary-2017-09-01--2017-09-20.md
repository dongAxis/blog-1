...
- [ ] model file, animation file format
- [ ] animation setup, progressing frame
- bin/CoreData/RenderPaths/Forward.xml
- model, material, (cast) shadow
- multiplayer mode architecture (network sync)
- rendering water
- rendering depth of field
- random idea
    - blender like viewport camera position change
...
'-' Node ("AdjNode")
  '-' AnimatedModel < StaticModel < Drawable < Component < ..
    ...
    '-* AnimationState
      ...
      '-* AnimationStateTrack
    '-* SourceBatch
      '-' Material ("mutant_M.xml") (TODO: so obviously Mutant.mdl knows how to UV map)
      '-' Geometry
      ...
      - CharacterDemo::Start =>
        ...
        - CreateCharacter =>
          ...
          - ResourceCache::GetResource<Model> "Models/Mutant/Mutant.mdl" =>
            - Resource::Load =>
              - Model::BeginLoad =>
                - parse .mdl file (verteces, indeces, geometries, morphs (what's this?))
                - Skelton::Load => list of Bone s
              - Model::EndLoad => VertexBuffer::SetData =>
                - Graphics::SetVBO => glBindBuffer
                - glBufferData ..
          - AnimatedModel::SetModel =>
            - StaticModel::SetNumGeometries => _batches.Resize ..
            - copy drawing data from Model
          - AnimatedMode::SetMaterial "mutant_M.xml" => ?? (upload texture ?)
            ...
            - AnimationController::PlayExclusive =>
              - Play =>
                ...
                - AddAnimationState => AnimatedModel::AddAnimationState =>
                  - AnimatedModel::AddAnimationState =>
                    - new AnimationState => SetStartBone ..
