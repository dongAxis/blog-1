...
- MidiTrack::roll =>
  - MidiDiskstream::process =>
    - _capture_buf->write (TODO: who does _capture_buf->read ?? is it butler ?? MidiDiskstream::do_flush ??)
    - signal DataRecorded(SMFSource)
    - get_playback => _playback_buf->read (TODO: who does _playback_buf->write ?? is it butler ?? MidiDiskstream::do_refill ??)
    ...
    - e.g. LuaPluginInfo::load =>
      - new LuaProc =>
        ...
        - load_script =>
          ...
          - get bindings to "dsp_run", "dsp_init", "dsp_params" ..
...
[ Butler ]
TODO diskstream read/write ringbuffer
```
