...
- I follow the engine code based on my workflow, which is as below
  - sync to first loop
  - quantize mode is loop
  so actually this mode completely ignores
...
- JackAudioDriver::_process_callback => Engine::process =>
  ...
  - handle rt event and run Looper instances alternatively based on event frame
    ...
    - Looper::do_event =>
      - ??
      - request_pending = true if any cmd to process (e.g. Event::Record)
    - Looper::run =>
      - ??
      - run_loops =>
